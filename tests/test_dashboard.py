"""Tests for the dashboard subprocess module and bot_manager status helpers.

The standalone ``abyss dashboard *`` subcommand was retired in
v2026.05.15 (folded into ``abyss start / restart / stop``). The lifecycle
helpers moved out of ``cli.py`` and into ``abyss.dashboard``; these
tests track the new module surface plus the ``_show_dashboard_status``
helper that still lives in ``bot_manager``.
"""
...
from abyss import dashboard


@pytest.fixture()
def temp_abyss_home(tmp_path, monkeypatch):
    """Set ABYSS_HOME to a temporary directory with one bot configured."""
    home = tmp_path / ".abyss"
    home.mkdir()
...
# --- abyss.dashboard.is_port_in_use ---
...
def test_is_port_in_use_with_listening_socket():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        assert dashboard.is_port_in_use(port) is True
...
def test_is_port_in_use_with_free_port():
    ...
    assert dashboard.is_port_in_use(port) is False


# --- abyss.dashboard.is_running ---


def test_is_running_with_valid_pid_file(temp_abyss_home):
    """Should detect running dashboard from PID file."""
    pid_file = temp_abyss_home / "abysscope.pid"
    pid_file.write_text(f"{os.getpid()}\n3847\n")

    running, pid = dashboard.is_running()
    assert running is True
    assert pid == os.getpid()


def test_is_running_with_stale_pid_file(temp_abyss_home):
    """Stale PID file with dead process should be cleaned up."""
    pid_file = temp_abyss_home / "abysscope.pid"
    pid_file.write_text("999999\n3847\n")

    with patch("abyss.dashboard.is_port_in_use", return_value=False):
        running, pid = dashboard.is_running()
...
def test_is_running_no_pid_file_port_in_use(temp_abyss_home):
    """Should detect dashboard via port fallback when no PID file exists."""
    with patch("abyss.dashboard.is_port_in_use", return_value=True):
        running, pid = dashboard.is_running()
...
def test_is_running_no_pid_file_port_free(temp_abyss_home):
...
# --- abyss.dashboard.get_port ---


def test_get_port_with_port_in_file(temp_abyss_home):
    """Should read port from second line of PID file."""
    pid_file = temp_abyss_home / "abysscope.pid"
    pid_file.write_text(f"{os.getpid()}\n4000\n")

    assert dashboard.get_port() == 4000


def test_get_port_single_line(temp_abyss_home):
    """Should return the default port when the PID file has only one line."""
    pid_file = temp_abyss_home / "abysscope.pid"
    pid_file.write_text(f"{os.getpid()}\n")

    assert dashboard.get_port() == dashboard.DEFAULT_PORT


def test_get_port_no_file(temp_abyss_home):
    """Should return None when no PID file exists."""
    assert dashboard.get_port() is None


# --- abyss.dashboard.stop_running ---


def test_stop_running_no_op_when_pid_file_missing(temp_abyss_home):
    """``stop_running`` on a non-running dashboard is a no-op."""
    with patch("abyss.dashboard.is_port_in_use", return_value=False):
        assert dashboard.stop_running() is None


def test_stop_running_returns_pid_for_running_dashboard(temp_abyss_home, monkeypatch):
    """When a PID file is live, ``stop_running`` returns the PID."""
    pid_file = temp_abyss_home / "abysscope.pid"
    fake_pid = os.getpid()
    ...
    def _kill(pid, sig):  # noqa: ARG001 - matches signature
        killed["pid"] = pid
    ...
    monkeypatch.setattr("os.getpgid", lambda _pid: _pid)

    assert dashboard.stop_running() == fake_pid
    assert not pid_file.exists()


# --- abyss.dashboard formatting helpers ---


def test_format_size_boundaries():
    assert dashboard._format_size(0) == "0 B"
    assert dashboard._format_size(512) == "512 B"
    assert dashboard._format_size(1024) == "1.0 KB"
    assert dashboard._format_size(1024 * 1024) == "1.0 MB"
    assert dashboard._format_size(1024 * 1024 * 1024) == "1.00 GB"
    assert dashboard._format_size(1024 * 1024 * 1024 * 5) == "5.00 GB"
...
def test_format_directory_relative_when_inside_cwd(tmp_path, monkeypatch):
    ...
    sub.mkdir()
    assert dashboard._format_directory(sub) == "abysscope"
...
def test_format_directory_absolute_when_outside_cwd(tmp_path, monkeypatch):
    ...
    assert dashboard._format_directory(other) == str(other.resolve())


def test_node_modules_present_detection(tmp_path):
    assert dashboard._node_modules_present(tmp_path) is False
    (tmp_path / "node_modules").mkdir()
    assert dashboard._node_modules_present(tmp_path) is True


def test_run_to_log_writes_command_header_and_returns_exit_code(tmp_path):
    log = tmp_path / "build.log"
    code = dashboard._run_to_log(["true"], cwd=tmp_path, env=os.environ.copy(), log_path=log)
    assert code == 0
...
def test_next_build_artifact_size_sums_files(tmp_path):
    assert dashboard._next_build_artifact_size(tmp_path) == 0  # no .next yet
    ...
    assert dashboard._next_build_artifact_size(tmp_path) == 3 * 1000 + 1
...
def test_show_dashboard_status_port_fallback(temp_abyss_home, capsys):
    """Should detect dashboard via port when no PID file is present."""
    from abyss.bot_manager import _show_dashboard_status
...
def test_show_dashboard_status_not_running(temp_abyss_home, capsys):
    """Should show not-running when no PID file and the port is free."""
    from abyss.bot_manager import _show_dashboard_status
...
# --- bot_manager._ensure_conversation_index ---
...
# --- abyss start / stop / restart CLI surface (post-merge) ---


def test_dashboard_subcommand_no_longer_exists():
    """``abyss dashboard *`` was retired — invoking it must fail."""
    ...
    # Typer reports unknown commands via stderr/help. We just need to
