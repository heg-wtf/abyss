...
### 38-40. OpenRouter / OpenAI-compat (RETIRED)

- **Status**: N/A — removed in v2026.05.15
- **Description**: The OpenRouter / OpenAI-compatible backends and their associated third-party API key handling, conversation egress to external providers, and markdown-echo surface no longer exist. Bots that still reference these backend types refuse to start with a migration hint. The third-party data-egress surface (findings 38 / 39) and Telegram-era markdown echo path (finding 40) are gone with them.
