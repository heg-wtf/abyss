# Abysscope: abyss Web Dashboard

> Plan created: 2026-03-11
> Branch: `feature/abysscope`

## Overview

Abysscope is a web-based dashboard for managing `~/.abyss/` configuration, bots, skills, cron jobs, sessions, and logs. Designed for **general users** who shouldn't need to open a terminal or text editor to manage their abyss setup.

## Problem

Currently managing abyss requires:
- Manually editing YAML files (`bot.yaml`, `config.yaml`, `cron.yaml`, `skill.yaml`)
- Terminal commands (`abyss bot add`, `abyss start`, `abyss stop`)
...
| State | Server-side file reads | No database needed, reads ~/.abyss directly |
| Package | npx-runnable | `npx abysscope` or bundled with abyss |
...
├── abyss.pid                      # Process ID (running state)
│
├── bots/{name}/
...
└── logs/
    └── abyss-YYMMDD.log           # Daily rotating logs
```
...
|-----|---------|-------|--------|------|-----------|
| abyss0 | (none) | opus | imessage, reminders, naver-map, image | - | - |
| abysslifebot | 앤 | sonnet | imessage, reminders, best-price, gmail, gcalendar, naver-search, kakao-local, daiso | 2 jobs | off |
| heg-staff | 킴 | opus | supabase, reminders, naver-search, mustgo-operation | - | off |
| abyssnotifybot | (none) | sonnet | clien | 2 jobs | off |
| abyssfinancebot | (none) | sonnet | dart | - | off |
...
- **Bot Cards**: Each bot as a card with display name, personality snippet, model badge, skill count, status indicator (running via abyss.pid check)
- **Quick Stats**: Total bots, active skills, cron jobs today, last activity timestamp
- **System Status**: abyss process running/stopped, bridge status, timezone/language
...
All API routes read/write directly to `~/.abyss/` filesystem.
...
│   │   ├── favicon.ico             # abyss logo favicon
│   │   ├── bots/
│   │   │   └── [name]/
...
│   │
│   └── lib/
│       ├── abyss.ts                # ~/.abyss filesystem reader/writer
...
- [x] ~/.abyss filesystem reader library (`lib/abyss.ts`)
...
1. **Embedded in abyss repo** — `abysscope/` directory at project root. Easier context management
2. **`abyss dashboard` command** — Added to CLI, spawns Next.js dev server
