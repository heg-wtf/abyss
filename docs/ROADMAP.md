...
## Now — v2026.05.19 (#122, #123, #124 머지 직후)

- **공진화 Phase 1 / 2a / 2b 완료**. 사람 → 봇 시그널 (숫자 피드백 1/2/3) + 봇 → 사람 시그널 (`about_me_propose` MCP tool + `/about-me` 승인 UI) 양방향 채널이 열렸다. 실 사용으로 누적 시그널을 모아 Phase 3 (`SELF.md` + reflection cron) 와 향후 DPO 파이프라인의 연료로 쓴다. 설계 / 진행 상황은 `docs/plan-coevolution-2026-05-19.md`.
...
- **Routines 탭 답장 검증**. PR #61 으로 cron / heartbeat 세션에 사용자가 답장하는 경로가 열렸다. 실 사용으로 SDK pool resume + `human` role 필터가 의도대로 도는지 누적 검증 필요.
...
- **공진화 Phase 3 — SELF.md + reflection**. 봇별 자기반성 markdown + 주간 reflection cron. 사용자의 1/2/3 피드백 + 거부된 ABOUT_ME propose 가 자기보정 패턴으로 누적. ABOUT_ME 와 동일한 propose → 사용자 승인 흐름 또는 자동 누적.
- **Chat-side propose 알림 badge**. 사이드바 헤더에 "n proposals pending" 표시 + click → `/about-me`. 폴링 30s. small follow-up.
- **PWA 위에서 다중-봇 협업 다시 설계**. 기존 group surface 는 v2026.05.14 에 제거. 후속 모델 후보:
...
- **임베딩 hybrid retrieval**. `conversation.db` 에 vector 컬럼 + local sentence-transformers. BM25 + semantic 결합. 한국어 nori / mecab 형태소 분석.
- **Memory git versioning**. `~/.abyss/` auto-commit cron → rollback 가능.
- **Privacy audit log**. 봇이 접근한 파일·MCP·URL jsonl + dashboard 검토 surface.
- **Attention rhythm 학습**. heartbeat 의 `active_hours` 자동 튜닝 — 사용자 응답 latency 패턴 학습.
- **Slash command UX 강화**. 자동완성, 인자 힌트, 명령별 미리보기. 모바일에 ⌘ 아이콘 추가 후 다음 단계.
- **검색 통합**. FTS5 (`conversation_search`) + QMD 콜렉션 + ABOUT_ME 가 별도로 동작 중. PWA 에서 통합 검색 surface — 채팅 / Routines / 메모리 한 번에.
