...
| `self_reflection.py` | Per-bot self-reflection notebook at `bots/<name>/SELF.md`. `load_self_md`, `save_self_md` (atomic write + `.prev` rollback, `MAX_SELF_BYTES = 8 KiB`), `ensure_self_scaffold`, `build_reflection_prompt` (combines recent conversation tails + `feedback.aggregate(last_n=50)` + current SELF.md), and async `run_reflection(bot, bot_config)` that calls `llm.registry.get_or_create(bot).run(LLMRequest(...))` and persists the new SELF.md. `compose_claude_md()` injects the file between `## Rules` and `## About Me`. Surfaced via `abyss self show/reflect/schedule/unschedule`, dashboard `/self`, and `GET/PUT /self/{bot}` |
| `episodes.py` | Phase 4 episodic-semantic store. `bots/<name>/episodes.jsonl` (append-only timeline) + `bots/<name>/facts.db` (SQLite claims with `(subject, claim)` unique index, confidence-monotone upsert, retract → status='retracted' + confidence=0). `extract_yesterday(bot, cfg)` reads every `conversation-YYMMDD.md` under the bot for the date, runs the bot's LLM with a schema-locked extraction prompt, and persists episodes + facts atomically. Dispatched by `cron.execute_cron_job` when the cron entry name is `EPISODE_EXTRACT_JOB_NAME = "episode_extract"`. Surfaced via `abyss episodes show/extract/schedule/unschedule`, `abyss facts show/retract`, dashboard `/episodes` + `/facts`, and `GET /episodes/{bot}` / `GET /facts/{bot}` / `PUT /facts/{bot}/{id}` |
| `skill_proposals.py` | Phase 5 skill-autonomy store at `bots/<name>/skill_proposals.yaml`. `Proposal` dataclass + `add_proposal` (dedup by `candidate_url`, preserves rejected/approved status), `list_proposals`, `update_status`, `approve(bot, id)` (chains `import_skill_from_github` → `attach_skill_to_bot` → mark approved, returns structured `{ok, error, stage}`), `reject`. Surfaced via `abyss skills proposals show/approve/reject`, dashboard `/skills/proposals`, and `GET /skill-proposals/{bot}` / `POST /skill-proposals/{bot}/{id}/approve` / `POST /skill-proposals/{bot}/{id}/reject` |
| `goals.py` | Phase 6 per-bot goal tracker at `bots/<name>/goals.yaml`. `Goal` + `ProgressEntry` dataclasses, CRUD (`add_goal` / `update_goal` / `delete_goal` / `mark_done` / `mark_archived`), `record_progress` (append-only timeline, never deduped), and `build_digest_prompt` for the weekly digest cron. `GOAL_DIGEST_JOB_NAME = "goal_digest"` is dispatched by `cron.execute_cron_job`. `compose_claude_md` injects the top-3 active goals so the bot keeps them in mind. Surfaced via `abyss goals show/add/progress/done/archive/delete/schedule/unschedule`, dashboard `/goals`, and `GET/POST/PUT/DELETE /goals/{bot}` + `POST /goals/{bot}/{id}/progress` |
| `persona_drift.py` | Phase 8.0 per-bot drift tracker. `bots/<name>/persona_snapshots.jsonl` stores `{ts, hash, total_bytes, section_sizes, event}` rows; only sha256 + sizes are kept (no body) for disk + privacy. `take_snapshot(bot, event)` composes CLAUDE.md and appends one row. `compute_drift(bot, window_days)` returns a `DriftReport` (baseline vs latest deltas + `shrinkage_alert` when total bytes drop past 10%). `PERSONA_DAILY_JOB_NAME` / `PERSONA_DIGEST_JOB_NAME` are dispatched by `cron.execute_cron_job`. Surfaced via `abyss persona show/snapshot/drift/schedule/unschedule`, dashboard `/persona`, and `GET /persona/{bot}/snapshots`, `GET /persona/{bot}/drift`, `POST /persona/{bot}/snapshot` |
...
| `mcp_servers/recall_fact.py` | stdio MCP server exposing `recall_fact(subject, k, min_confidence)` + `recent_episodes(days, limit, kind)`. Auto-injected by `claude_runner` when the bot has a `facts.db`. Resolves the bot from cwd (same heuristic as `conversation_search`) |
| `mcp_servers/propose_skill.py` | stdio MCP server with one tool — `propose_skill(candidate_url, reason, alternative_urls?)`. Always-on per bot (no gate condition). Validates URL is `https://github.com/<owner>/<repo>`, calls `skill_proposals.add_proposal` (dedup against pending / approved / rejected), and fires a best-effort Web Push so the human notices the queue grew |
| `mcp_servers/call_bot.py` | Phase 7.0 stdio MCP server with one tool — `call_bot(bot, message, timeout?)`. Always-on per bot. Resolves the caller from cwd, refuses self-calls, refuses unknown peers, refuses messages over 4 KB, and refuses to recurse past `MAX_DEPTH = 3` (tracked via `ABYSS_CALL_BOT_DEPTH` env var that increments + restores around each call). Peer runs through `llm.registry.get_or_create` with a fresh `session_key=peer_call:<caller>:<peer>:<depth>` and a per-call working directory `bots/<peer>/peer_call_sessions/from_<caller>/` so audit logs stay separate from the peer's own chat history |
| `mcp_servers/record_progress.py` | Phase 6 stdio MCP server with one tool — `record_progress(goal_id, note, value?)`. Always-on per bot. Resolves the caller from cwd, rejects empty notes, rejects notes over 2 KB, rejects non-numeric values, and refuses to invent new goals — the goal must already exist in `goals.yaml`. Appends to the goal's timeline atomically |
...
| `hooks/postcompact_hook.py` | Phase 8.0 Claude Code `PostCompact` hook — takes a `persona_drift` snapshot tagged `post-compact` and fires a Web Push if composed CLAUDE.md shrank past the threshold. Never blocks (always exits 0) |
...
- Workspace panel: `WorkspaceTree` (chat side panel) lazy-loads `~/.abyss/bots/<bot>/sessions/<chat>/workspace/` via `/api/chat/workspace`. Clicking a previewable file (`.md`, `.txt`, `.log`, `.yaml`, `.json`, `.sh`, `.py`, `.ts`, `.tsx`, `.js`, `.jsx`, `.toml`) opens an inline panel-internal preview (back button) that hits `/api/chat/workspace/file` — capped at `MAX_PREVIEW_BYTES = 1 MiB` with realpath/symlink-escape guards. Markdown renders through `react-markdown` + `remark-gfm` + `remark-breaks` + `rehype-highlight` (highlight.js `github-dark` theme imported once in `globals.css`); other text falls through to a syntax-plain `<pre>`. The session-id handling normalises a leading `chat_` so mobile chat IDs that already include the prefix don't double-up
